...
NetworkState_t network_state = NETWORK_INIT;
...
#ifdef JC_EXPAND 
static Rx2ChannelParams_t Rx4Channel = RX_WND_4_CHANNEL;
#endif
/*!
...
{
    int32_t delay;
    int32_t curTime = ICall_getTicks() / 100;//TimerGetCurrentTime( );
    System_printf("OnRadioTxDone %d\r\n", ICall_getTicks() / 100);
    ...
    {
#ifdef JC_EXPAND
        rx1_window_time = ICall_getTicks() / 100;
#endif
        Util_restartClock(&RxWindowTimer1, RxWindow1Delay - 5);   
//          System_printf("RxWindowTimer1 %d , %d\r\n", RxWindow1Delay - 10, ICall_getTicks() / 100);
        if( LoRaMacDeviceClass != CLASS_C )
        {
            Util_restartClock(&RxWindowTimer2, RxWindow2Delay);
//            System_printf("RxWindowTimer2 %d , %d\r\n", RxWindow2Delay + 10, ICall_getTicks() / 100);
        }
        if( ( LoRaMacDeviceClass == CLASS_C ) || ( NodeAckRequested == true ) )
        {   
            delay = RxWindow2Delay + ACK_TIMEOUT + randr( -ACK_TIMEOUT_RND, ACK_TIMEOUT_RND );
            Util_restartClock(&AckTimeoutTimer, delay);
 //           System_printf("AckTimeoutTimer %d , %d\r\n", delay, ICall_getTicks() / 100);
...
static void PrepareRxDoneAbort( void )
{
 ...
 //   TimerStart( &MacStateCheckTimer );
	Util_restartClock(&MacStateCheckTimer, 1);
//    System_printf("MacStateCheckTimer %d , %d\r\n", 10, ICall_getTicks() / 100);
}

#ifdef JC_EXPAND
uint32_t dr0_5_interval[6];
uint32_t local_time;
uint32_t system_time;
uint32_t r3_interval;
uint32_t r4_interval;
uint32_t r4_interval_offset;
uint32_t recive_timer_time;
uint32_t last_beacon_time;
uint32_t rx1_window_time;

static void Restart_R3_R4_timer(void)
{
    uint32_t temp;
    
    last_beacon_time = ICall_getTicks() / 100;
    
    Util_stopClock(&RxWindowTimer4);
    
    if (network_state != NETWORK_JION) {
        network_state = NETWORK_JION;  
    }
    
    if( IsRxWindowsEnabled == true )
    {
        temp = r4_interval - (system_time % r4_interval) + r4_interval_offset;
        Util_restartClock(&RxWindowTimer4, temp); 
        recive_timer_time = (ICall_getTicks() / 100) + temp;
        System_printf("offset : %d\r\n", r4_interval_offset); 
    }
}

static void R3_R4_timer_syn(uint8_t *payload, uint32_t TimeOnAir)
{
    uint8_t i;
    
    system_time =  payload[0];
    system_time += payload[1] << 8;
    system_time += payload[2] << 16;
    system_time += payload[3] << 24;
    r3_interval =  payload[4] * 1000;              
    dr0_5_interval[0] = payload[5] * 100;
    dr0_5_interval[1] = payload[6] * 100;
    dr0_5_interval[2] = payload[7] * 100;
    dr0_5_interval[3] = payload[8] * 100;
    dr0_5_interval[4] = payload[9] * 100;
    dr0_5_interval[5] = payload[10] * 100;

    if (LORAWAN_ADR_ON == true && LoRaMacDeviceClass == CLASS_D) {
        if (payload[11] & 0x40) {
            System_printf("adr on\r\n");         
            AdrCtrlOn = true;
        } else {
            AdrCtrlOn = false;
            for (i = 0; i <= DR_5; i++) {
    ...
    if (payload[12]) {
        ChannelsMask[0] = payload[12];
    }
    
    Rx4Channel.Datarate = ChannelsDatarate;
    switch (ChannelsDatarate) {
        case DR_0:  
            r4_interval = dr0_5_interval[ChannelsDatarate];
            r4_interval_offset = 0;
            Rx4Channel.Frequency = FRQ_DR_0;
            break;
        case DR_1:
            r4_interval = dr0_5_interval[ChannelsDatarate];
            r4_interval_offset = 0;
            Rx4Channel.Frequency = FRQ_DR_1;
            break;
        case DR_2:
            r4_interval = dr0_5_interval[ChannelsDatarate];
            r4_interval_offset = 0;           
            Rx4Channel.Frequency = FRQ_DR_2;
            break;
        case DR_3:
            r4_interval = dr0_5_interval[DR_5];
            r4_interval_offset = dr0_5_interval[ChannelsDatarate];
            Rx4Channel.Frequency = FRQ_DR_3;
            break;
        case DR_4:
            r4_interval = dr0_5_interval[DR_5];
            r4_interval_offset = dr0_5_interval[ChannelsDatarate];
            Rx4Channel.Frequency = FRQ_DR_4;
            break;
        case DR_5:    
            r4_interval = dr0_5_interval[ChannelsDatarate];
            Rx4Channel.Frequency = FRQ_DR_5;
            r4_interval_offset = 0;
            break;
        default:
            break;       
    }
    local_time = ICall_getTicks() / 100;
    TimeOnAir += 8;
...
static void OnRadioRxDone( uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr )
{
    ...
    System_printf("RxDone %d, %d\r\n", ICall_getTicks() / 100, ChannelsDatarate);

    TimeOnAir = Radio.TimeOnAir( MODEM_LORA, size);
    
#ifdef UART_PRINT_ENABLE    
    System_printf("len : %d, TimeOnAir : %d, rssi %d\r\n", size, TimeOnAir, rssi);
#endif  
...
//    TimerStop( &RxWindowTimer2 );
    Util_stopClock( &RxWindowTimer2 );

    macHdr.Value = payload[pktHeaderLen++];
    
#if 1
    System_printf("Rx %d B :", size);
    for (i = 0; i < size; i++) {
        System_printf(" %02x", payload[i]);
    }       
    System_printf("\r\n");
#endif  
    ...
    switch( macHdr.Bits.MType )
    {
        case FRAME_TYPE_JOIN_ACCEPT:
            ...
            System_printf("Rx Decrypt%d B :", size);
            for (i = 0; i < size; i++) {
                System_printf(" %02x", LoRaMacRxPayload[i]);
            }       
            System_printf("\r\n");
#endif
...
#ifdef JC_EXPAND
                R3_R4_timer_syn(&payload[size], TimeOnAir);
#endif
#if 0
                System_printf("LoRaMacNwkSKey :");
                for (i = 0; i < 16; i++) {
                    System_printf(" %02x", LoRaMacNwkSKey[i]);
                }       
                System_printf("\r\n");
                
                            
                System_printf("LoRaMacAppSKey :");
                for (i = 0; i < 16; i++) {
                    System_printf(" %02x", LoRaMacAppSKey[i]);
                }       
                System_printf("\r\n");
                
                System_printf("LoRaMacNetID   : %06x\r\n", LoRaMacNetID);
                
                System_printf("LoRaMacDevAddr : %08x\r\n", LoRaMacDevAddr);
                
                System_printf("Rx1DrOffset    : %02x\r\n", Rx1DrOffset);

                System_printf("Rx2Datarate    : %02x\r\n", Rx2Channel.Datarate);
#endif
#if defined( USE_BAND_915 ) || defined( USE_BAND_915_HYBRID )
                /*
 ...
 //                   ReceiveDelay1 = 2;
                ...
                ReceiveDelay1 *= 1000;
                ReceiveDelay2 = ReceiveDelay1 + 1000;
                System_printf("ReceiveDelay1 %d\r\n", ReceiveDelay1);
        ...
        case FRAME_TYPE_DATA_UNCONFIRMED_DOWN:
            ...
            for (i = 1; i <= frame_count; i++) {
                ...
                if (frame_count > 1 && i < frame_count) {
                    while (pktHeaderLen < size) {
                        if (payload[pktHeaderLen++] == 0xFA && payload[pktHeaderLen++] == 0xAF) {
                             break;
                        }
                    }
                }
                ...
                if (is_my_frame) {
                    ...
                    //最后一个报文
                        ...
                        pktHeaderLen = frame_start + 5;
                    }
                    frame_end = frame_start + frame_size - 1;
                    
                    System_printf("frame_end %d, frame_start %d ,  frame_size %d\r\n", frame_end, frame_start, frame_size);
                    break;
                }
            }
            ...
            appPayloadStartIndex = frame_start + 8 + fCtrl.Bits.FOptsLen;

            micRx |= ( uint32_t )payload[frame_end - LORAMAC_MFR_LEN + 1];
            micRx |= ( ( uint32_t )payload[frame_end - LORAMAC_MFR_LEN + 2] << 8 );
            micRx |= ( ( uint32_t )payload[frame_end - LORAMAC_MFR_LEN + 3] << 16 );
            micRx |= ( ( uint32_t )payload[frame_end - LORAMAC_MFR_LEN + 4] << 24 );
            ...
            if( sequenceCounterDiff < ( 1 << 15 ) )
            {
                ...
                LoRaMacComputeMic( &payload[frame_start], frame_size - LORAMAC_MFR_LEN, nwkSKey, address, DOWN_LINK, downLinkCounter, &mic );
                if( micRx == mic )
                {
                    isMicOk = true;
                } else {
//                        System_printf("Mic fail!\r\n");
                }
            }
            else
            {
                ...
                LoRaMacComputeMic( &payload[frame_start], frame_size - LORAMAC_MFR_LEN, nwkSKey, address, DOWN_LINK, downLinkCounterTmp, &mic );
                if( micRx == mic )
                {
                    isMicOk = true;
...
//                       System_printf("Mic fail!\r\n");
            ...
            if( isMicOk == true )
            {
              ...
              // Update 32 bits downlink counter
                ...
                if( fCtrl.Bits.Ack == 1 )
                {                       
//                       System_printf("fCtrl.Bits.Ack == 1!\r\n");
...
//                       TimerStop( &AckTimeoutTimer );
                    Util_stopClock(&AckTimeoutTimer);
                }
                else
                {
                    McpsConfirm.AckReceived = false;
...
//                           TimerStop( &AckTimeoutTimer );                            
                ...
                if( fCtrl.Bits.FOptsLen > 0 )
                {
                    ...
                    ProcessMacCommands( payload, frame_start + 8, appPayloadStartIndex, snr );
                }
                if( ( ( frame_end - 3 ) - appPayloadStartIndex ) > 0 )
                {                       
                    port = payload[appPayloadStartIndex++];
                    frameLen = ( frame_end - 3 ) - appPayloadStartIndex;

                    McpsIndication.Port = port;

                    if( port == 0 )
                    {                           
 //                       System_printf("ProcessMacCommands \r\n");
                    ...
                    {                            
                        ...
                        //System_printf("LoRaMacPayloadDecrypt%d B :", frameLen);
        ...
        case FRAME_TYPE_PROPRIETARY:
            {          
#ifdef JC_EXPAND
                if ((payload[1] != 0x1) && (CRC8_Table(payload, 10) != payload[10])) {
                    System_printf("crc error!\r\n");
                    break;
                }

                R3_R4_timer_syn(&payload[11], TimeOnAir);   
                
                System_printf("recive beacon\r\n");
#endif
                //System_printf("FRAME_TYPE_PROPRIETARY recived \r\n");
...
#ifdef JC_EXPAND 
    if ( LoRaMacDeviceClass == CLASS_D ) {
        if ((last_beacon_time != 0) && ((ICall_getTicks() / 100) - last_beacon_time  > 2 * r3_interval)) {
             if (network_state != NETWORK_LOST_BEACON) {
                 network_state = NETWORK_LOST_BEACON;                
                 System_printf("last_beacon_time %d ,current_time %d\r\n", last_beacon_time, ICall_getTicks() / 100);
                 RetartTxNextPacketTimer(true, 15000 + randr(5000,20000));
             }
        }
    }
#endif
...
//    Util_stopClock(&MacStateCheckTimer);
    bool txTimeout = false;
    
//    System_printf("MacStateCheckTimer1 %d\r\n", ICall_getTicks() / 100);
 ...
 //                   LoRaMacState &= ~MAC_TX_RUNNING;
        ...
        if( ( AckTimeoutRetry == true ) && ( ( LoRaMacState & MAC_TX_DELAYED ) == 0 ) )
        {
            ...
            if( ( AckTimeoutRetriesCounter < AckTimeoutRetries ) && ( AckTimeoutRetriesCounter <= MAX_ACK_RETRIES ) )
            {
                ...
                LoRaMacFlags.Bits.MacDone = 0;
                // Sends the same frame again 
//                printf("Sends comformed frame again!\r\n");
                ScheduleTx( );
            }
            else
            {
               ...
               // ChannelsMask[0] = ChannelsMask[0] | ( LC( 1 ));// + LC( 2 ) + LC( 3 ) );
                ChannelsMask[0] = ChannelsMask[0] | JOIN_CHANNELS;
...
//    #error "Please define a frequency band in the compiler options."
                ...
                if (network_state != NETWORK_SEND_FAIL) {
                    network_state = NETWORK_SEND_FAIL;  
                }
                RetartTxNextPacketTimer(true, 600000 + randr(10000,60000));
            }
        }
    }
    ...
    if( LoRaMacState == MAC_IDLE )
    {
        ...
        {            
            if (LoRaMacPrimitives->MacMcpsConfirm != NULL) {
                LoRaMacPrimitives->MacMcpsConfirm( &McpsConfirm );
            } else {
                //System_printf("LoRaMacPrimitives->MacMcpsConfirm = NULL\r\n");
            }
        ...
        if( LoRaMacFlags.Bits.MlmeReq == 1 )
        {
            if (LoRaMacPrimitives->MacMlmeConfirm != NULL) {
                ...
                //System_printf("LoRaMacPrimitives->MacMlmeConfirm = NULL\r\n");
            }
//            LoRaMacPrimitives->MacMlmeConfirm( &MlmeConfirm );
...
//        TimerStart( &MacStateCheckTimer );       
        Util_restartClock(&MacStateCheckTimer, MAC_STATE_CHECK_TIMEOUT);
        
//        System_printf("MacStateCheckTimer2 %d\r\n", ICall_getTicks() / 100);
    }
...
//        LoRaMacPrimitives->MacMcpsIndication( &McpsIndication );
        if (LoRaMacPrimitives->MacMcpsIndication != NULL) {
            ...
            //System_printf("LoRaMacPrimitives->MacMcpsIndication = NULL\r\n");
        }
...
{
    uint16_t symbTimeout = 10; // DR_2, DR_1, DR_0
    int8_t datarate = 0;
    uint32_t bandwidth = 0; // LoRa 125 kHz

//    TimerStop( &RxWindowTimer1 );
    
//    Util_stopClock(&RxWindowTimer1);
    RxSlot = 0;
    if( LoRaMacDeviceClass == CLASS_C )
    {
        Radio.Standby( );
...
//    System_printf("OnRxWindow1TimerEvent %d, datarate %d, bandwidth %d, symbTimeout %d\r\n", ICall_getTicks() / 100, datarate, bandwidth, symbTimeout);
    RxWindowSetup( Channels[Channel].Frequency + 5000000, datarate, bandwidth, symbTimeout, false);
//    RxWindowSetup( Channels[Channel].Frequency + 5000000, datarate, bandwidth, symbTimeout, false );
    System_printf("R1 %d, frq %d, datarate %d\r\n",
                 ICall_getTicks() / 100, Channels[Channel].Frequency + 5000000, datarate);
...
//    Util_stopClock( &RxWindowTimer2 );
    ...
    if( LoRaMacDeviceClass != CLASS_C )
    {
//        System_printf("OnRxWindow2TimerEvent %d, %d\r\n", ICall_getTicks() / 100, symbTimeout);
        RxWindowSetup( Rx2Channel.Frequency + 5000000, Rx2Channel.Datarate, bandwidth, symbTimeout, false );
//		RxWindowSetup( Rx2Channel.Frequency, Rx2Channel.Datarate, bandwidth, symbTimeout, true );
        System_printf("R2 %d, frq %d, dr %d\r\n",
                      ICall_getTicks() / 100, Rx2Channel.Frequency + 5000000, Rx2Channel.Datarate);

    }
    else
    {               
        RxWindowSetup( Rx2Channel.Frequency + 5000000, Rx2Channel.Datarate, bandwidth, symbTimeout, true );
//		RxWindowSetup( Rx2Channel.Frequency, Rx2Channel.Datarate, bandwidth, symbTimeout, true );
    }
}
...
void OnRxWindow4TimerEvent( void )
{
    uint16_t symbTimeout = 5; // DR_2, DR_1, DR_0
    uint32_t bandwidth = 0; // LoRa 125 kHz
...
#if defined( USE_BAND_433 ) || defined( USE_BAND_780 ) || defined( USE_BAND_868 )
    // For higher datarates, we increase the number of symbols generating a Rx Timeout
    if ( Rx4Channel.Datarate >= DR_3 ) { // DR_6, DR_5, DR_4, DR_3
        symbTimeout = 10;
    }
    
    if ( Rx4Channel.Datarate == DR_5 ) { // DR_6, DR_5, DR_4, DR_3
        symbTimeout = 20;
    } else if ( Rx4Channel.Datarate == DR_6 ) {// LoRa 250 kHz
        bandwidth  = 1;      
        symbTimeout = 20;       
    }
#elif ( defined( USE_BAND_915 ) || defined( USE_BAND_915_HYBRID ) )
    // For higher datarates, we increase the number of symbols generating a Rx Timeout
    if( Rx4Channel.Datarate > DR_0 )
    { // DR_1, DR_2, DR_3, DR_4, DR_8, DR_9, DR_10, DR_11, DR_12, DR_13
        symbTimeout = 10;
    }
    if( Rx4Channel.Datarate >= DR_4 )
    {// LoRa 500 kHz
        bandwidth  = 10;
    }
#else
    #error "Please define a frequency band in the compiler options."
#endif
    RxWindowSetup( Rx4Channel.Frequency + 5000000, Rx4Channel.Datarate, bandwidth, symbTimeout, false );

    System_printf("R4 %d, frq %d, dr %d\r\n",
                  ICall_getTicks() / 100, Rx4Channel.Frequency + 5000000, Rx4Channel.Datarate);
...
//    Util_stopClock( &AckTimeoutTimer );
    //System_printf("OnAckTimeoutTimerEvent %d\r\n", ICall_getTicks() / 100);
    if( NodeAckRequested == true )
    {
...
static bool SetNextChannel( TimerTime_t* time )
{
    ...
    uint32_t curTime = ICall_getTicks() / 100;
...
static void RxWindowSetup( uint32_t freq, int8_t datarate, uint32_t bandwidth, uint16_t timeout, bool rxContinuous )
{
    ...
    if( Radio.GetStatus( ) == RF_IDLE )
    {
        ...
        {
            modem = MODEM_LORA;
 //           Radio.SetRxConfig( modem, bandwidth, downlinkDatarate, 1, 0, 8, timeout, false, 0, true, 0, 0, false, rxContinuous );
            Radio.SetRxConfig( modem, bandwidth, downlinkDatarate, 1, 0, 8, timeout, false, 0, true, 0, 0, true, rxContinuous );
        }
#elif defined( USE_BAND_915 ) || defined( USE_BAND_915_HYBRID )
        modem = MODEM_LORA;
...
static void ProcessMacCommands( uint8_t *payload, uint8_t macIndex, uint8_t commandsSize, uint8_t snr )
{
    ...
    #error "Please define a frequency band in the compiler options."
                    ...
                    if( ( status & 0x07 ) == 0x07 )
                    {
                        System_printf("SRV_MAC_LINK_ADR_REQ, datarate %d, txPower %d\r\n", datarate, txPower);
...
//        TimerStart( &TxDelayedTimer );
        Util_restartClock(&TxDelayedTimer, dutyCycleTimeOff);
        //  //System_printf("TxDelayedTimer %d, %d\r\n", dutyCycleTimeOff, ICall_getTicks() / 100);
...
LoRaMacStatus_t SendFrameOnChannel( ChannelParams_t channel )
{
    ...
    { // High Speed FSK channel
        ...
        Radio.SetTxConfig( MODEM_FSK, txPower, 25000, 0, datarate * 1000, 0, 5, false, true, 0, 0, false, 3e3 );
...
//        Radio.SetTxConfig( MODEM_LORA, txPower, 0, 1, datarate, 1, 8, false, true, 0, 0, false, 3e6 );
    ...
    { // Normal LoRa channel
        System_printf("Frequency = %d, txpower %d, datarate %d\r\n", channel.Frequency, txPower, datarate);
        Radio.SetMaxPayloadLength( MODEM_LORA, LoRaMacBufferPktLen );
...
//    TimerStart( &MacStateCheckTimer );
    ...
    //System_printf("MacStateCheckTimer %d\r\n", ICall_getTicks() / 100);
  
    // Send now
    Radio.Send( LoRaMacBuffer, LoRaMacBufferPktLen );
#if 1    
    System_printf("send %d B :", LoRaMacBufferPktLen);
    for (int i = 0; i < LoRaMacBufferPktLen; i++) {
        System_printf(" %02x", LoRaMacBuffer[i]);
    }       
    System_printf("\r\n");
#endif    
...
LoRaMacStatus_t LoRaMacInitialization( LoRaMacPrimitives_t *primitives, LoRaMacCallback_t *callbacks )
{
    ...
    ChannelsMask[0] = JOIN_CHANNELS;//0x11FF;
#elif defined( USE_BAND_780 )
    ChannelsMask[0] = LC( 1 ) + LC( 2 ) + LC( 3 );
#elif defined( USE_BAND_868 )
    ChannelsMask[0] = LC( 1 ) + LC( 2 ) + LC( 3 );
...
LoRaMacStatus_t LoRaMacMlmeRequest( MlmeReq_t *mlmeRequest )
{
    ...
    {
//       LoRaMacState = MAC_IDLE;
//       printf("LoRaMacMlmeRequest MAC_TX_RUNNING\r\n");
       return LORAMAC_STATUS_BUSY;
