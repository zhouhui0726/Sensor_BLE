...
void SX1278FskSetRFFrequency( uint32_t freq )
{
    ...
    SX1278WriteBuffer( REG_FRFMSB, &SX1276->RegFrfMsb, 3 );

    SX1276->RegPaConfig = SX1278Read( REG_PACONFIG );
    ...
    if( FskSettings.RFFrequency > 860000000 )
    {
        ...
        SX1278Write( REG_PACONFIG, SX1276->RegPaConfig ); 

        if( SX1278FskGetPa20dBm( ) == true )
        {
...
{
    SX1278ReadBuffer( REG_FRFMSB, &SX1276->RegFrfMsb, 3 );
...
void SX1278FskSetBitrate( uint32_t bitrate )
{
    ...
    SX1278WriteBuffer( REG_BITRATEMSB, &SX1276->RegBitrateMsb, 2 );    
}

uint32_t SX1278FskGetBitrate( void )
{
    SX1278ReadBuffer( REG_BITRATEMSB, &SX1276->RegBitrateMsb, 2 );
...
void SX1278FskSetFdev( uint32_t fdev )
{
    ...
    SX1276->RegFdevMsb = SX1278Read( REG_FDEVMSB ); 
    ...
    SX1278WriteBuffer( REG_FDEVMSB, &SX1276->RegFdevMsb, 2 );    
}

uint32_t SX1278FskGetFdev( void )
{
    SX1278ReadBuffer( REG_FDEVMSB, &SX1276->RegFdevMsb, 2 );
...
{   
    SX1276->RegPaConfig = SX1278Read( REG_PACONFIG );
    SX1276->RegPaDac = SX1278Read( REG_PADAC );
...
{
    SX1276->RegPacketConfig1 = SX1278Read( REG_PACKETCONFIG1 );
    SX1276->RegPacketConfig1 = ( SX1276->RegPacketConfig1 & RF_PACKETCONFIG1_CRC_MASK ) | ( enable << 4 );
    SX1278Write( REG_PACKETCONFIG1, SX1276->RegPacketConfig1 );
...
{
    SX1276->RegRxConfig = SX1278Read( REG_RXCONFIG );
    SX1276->RegRxConfig = ( SX1276->RegRxConfig & RF_RXCONFIG_AFCAUTO_MASK ) | ( enable << 4 );
    SX1278Write( REG_RXCONFIG, SX1276->RegRxConfig );
...
void SX1278FskSetPayloadLength( uint8_t value )
{
    ...
    SX1278Write( REG_PAYLOADLENGTH, SX1276->RegPayloadLength );
    FskSettings.PayloadLength = value;
}

uint8_t SX1278FskGetPayloadLength( void )
{
    SX1276->RegPayloadLength = SX1278Read( REG_PAYLOADLENGTH );
...
void SX1278FskSetPa20dBm( bool enale )
{
    ...
    SX1278Write( REG_PADAC, SX1276->RegPaDac );
}
...
{
    SX1276->RegPaRamp = SX1278Read( REG_PARAMP );
...
{
    SX1276->RegRssiConfig = SX1278Read( REG_RSSICONFIG );
    if( offset < 0 )
    {
        offset = ( ~offset & 0x1F );
        offset += 1;
    ...
    SX1278Write( REG_RSSICONFIG, SX1276->RegRssiConfig );
}
...
int8_t SX1278FskGetRawTemp( void )
{
    ...
    SX1276->RegTemp = SX1278Read( REG_TEMP );
