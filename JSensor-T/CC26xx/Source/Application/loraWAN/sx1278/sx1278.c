...
void SX1278Init( RadioEvents_t *events )
{
    ...
    //System_printf("TxTimeoutTimer %0x\r\n", &TxTimeoutTimer);
    
    //System_printf("RxTimeoutTimer %0x\r\n", &RxTimeoutTimer);

    //System_printf("RxTimeoutSyncWord %0x\r\n", &RxTimeoutSyncWord);
#endif
    SX1276Reset( );
...
//            SX1278.Settings.LoRa.LowDatarateOptimize = 0x01;
#if 0
           System_printf("bandwidth = %d\r\n", bandwidth);
           System_printf("coderate = %d\r\n", coderate);
           System_printf("fixLen = %d\r\n", fixLen);
           System_printf("datarate = %d\r\n", datarate);
           System_printf("crcOn = %d\r\n", crcOn);
           System_printf("preambleLen = %d\r\n", preambleLen);
           System_printf("LowDatarateOptimize = %d\r\n", SX1278.Settings.LoRa.LowDatarateOptimize);
#endif
                        ...
                        uint8_t hopPeriod, bool iqInverted, uint32_t timeout )
{
    ...
    System_printf("power : %d, datarate : %d\r\n", power, datarate);
...
//    SX1278Write( REG_PACONFIG, 0xff );
//    SX1278Write( REG_PADAC, 0x87 );
//    System_printf("REG_PACONFIG %02x, REG_PADAC %02x, REG_LR_LNA %02x, REG_LR_MODEMCONFIG3 %02x\r\n", SX1278Read( REG_PACONFIG ), SX1278Read( REG_PADAC ), SX1278Read( REG_LR_LNA ), SX1278Read( REG_LR_MODEMCONFIG3 ));
 ...
 //           SX1278.Settings.LoRa.LowDatarateOptimize = 0x01;
            ...
            SX1278Write( REG_LR_MODEMCONFIG3, 
                         ( SX1278Read( REG_LR_MODEMCONFIG3 ) &
                           RFLR_MODEMCONFIG3_LOWDATARATEOPTIMIZE_MASK ) |
                           ( SX1278.Settings.LoRa.LowDatarateOptimize << 3 ) );
//            printf("REG_LR_MODEMCONFIG3 = 0x%02x", SX1278Read( REG_LR_MODEMCONFIG3 ));
...
#if 0
            System_printf("pktLen = %d\r\n", (uint32_t)(pktLen));
            System_printf("LowDatarateOptimize = %d\r\n", SX1278.Settings.LoRa.LowDatarateOptimize);
            System_printf("fixLen = %d\r\n", SX1278.Settings.LoRa.FixLen);
            System_printf("Datarate = %d\r\n", SX1278.Settings.LoRa.Datarate);
            System_printf("Coderate = %d\r\n", SX1278.Settings.LoRa.Coderate);
            System_printf("CrcOn = %d\r\n", SX1278.Settings.LoRa.CrcOn);
#endif
...
//    Util_stopClock(&RxTimeoutTimer);
//    Util_stopClock(&TxTimeoutTimer);
...
//        TimerStart( &RxTimeoutTimer );
        Util_restartClock(&RxTimeoutTimer, timeout);
//        System_printf("RxTimeoutTimer %d, %d\r\n", timeout, ICall_getTicks() / 100);
    }
    ...
    if( SX1278.Settings.Modem == MODEM_FSK )
    {
        ...
        {
            #if 0
            TimerSetValue( &RxTimeoutSyncWord, ( 8.0 * ( SX1278.Settings.Fsk.PreambleLen +
                                                         ( ( SX1278Read( REG_SYNCCONFIG ) &
                                                            ~RF_SYNCCONFIG_SYNCSIZE_MASK ) +
                                                         1.0 ) + 10.0 ) /
                                                        ( double )SX1278.Settings.Fsk.Datarate ) * 1e6 );
            TimerStart( &RxTimeoutSyncWord );

            #endif
            Util_restartClock(&RxTimeoutSyncWord, ( 8.0 * ( SX1278.Settings.Fsk.PreambleLen +
                                                         ( ( SX1278Read( REG_SYNCCONFIG ) &
                                                            ~RF_SYNCCONFIG_SYNCSIZE_MASK ) +
                                                         1.0 ) + 10.0 ) /
                                                        ( double )SX1278.Settings.Fsk.Datarate ) * 1000);
        }
    }
    else
    {
...
//    TimerStart( &TxTimeoutTimer );
    Util_restartClock(&TxTimeoutTimer, timeout);
    //  //System_printf("TxTimeoutTimer %d, %d\r\n", timeout, ICall_getTicks() / 100);
...
void SX1278SetOpMode( uint8_t opMode )
{
 //   static uint8_t opModePrev = RF_OPMODE_STANDBY;//    printf("OPMODE = 0x%2x", opMode);
    if (1)//( opMode != opModePrev )
    {
 //       opModePrev = opMode;
        ...
        SX1278Write( REG_OPMODE, (( SX1278Read( REG_OPMODE ) & RF_OPMODE_MASK ) | opMode ) & 0xf7);
        SX1278Read( REG_OPMODE );
    }
}
...
void SX1278WriteBuffer(uint8_t addr, uint8_t *buffer, uint8_t size)
{
		uint8_t addr_tmp = 0;
		addr_tmp = (addr | 0x80);
		bspSpiLoraSelect(true);
		bspSpiWrite(&addr_tmp, 1);
		bspSpiFlush();//Transfer Complete
		bspSpiWrite( buffer, size);
		bspSpiFlush();//Transfer Complete
		bspSpiLoraSelect(false);
}
...
//                TimerStop( &RxTimeoutSyncWord );
                ...
                Util_stopClock(&RxTimeoutSyncWord);
            }
        }
        if( ( RadioEvents != NULL ) && ( RadioEvents->RxTimeout != NULL ) )
        {
            
            //  //System_printf("RxTimeout %d\r\n", ICall_getTicks() / 100);
            RadioEvents->RxTimeout( );
            
            //  //System_printf("RxTimeout2\r\n");
        }
        break;
    case RF_TX_RUNNING:
        
        //  //System_printf("RF_TX_RUNNING\r\n");
        SX1278.Settings.State = RF_IDLE;
        if( ( RadioEvents != NULL ) && ( RadioEvents->TxTimeout != NULL ) )
        {
            //  //System_printf("TxTimeout\r\n");
...
//                            TimerStart( &RxTimeoutSyncWord );
                            ...
                            Util_startClock(&RxTimeoutSyncWord);
                        }
                        else
                        {
                            // Continuous mode restart Rx chain
...
//                TimerStop( &RxTimeoutTimer );
   ...
   //                 //System_printf("rx done %d\r\n", ICall_getTicks() / 100);
                    
                    
                    if( ( irqFlags & RFLR_IRQFLAGS_PAYLOADCRCERROR_MASK ) == RFLR_IRQFLAGS_PAYLOADCRCERROR )
                    {                        
                        //System_printf("rx crc error\r\n");
                        // Clear Irq
...
//            TimerStop( &TxTimeoutTimer );
 ...
 //                 //System_printf("tx done %d\r\n", ICall_getTicks() / 100);
            ...
            default:
                SX1278.Settings.State = RF_IDLE;
   //             if ( RadioEvents == NULL ) 
                      //System_printf("RadioEvents == NULL\r\n");
  //              if ( RadioEvents->TxDone == NULL ) 
                      //System_printf("RadioEvents->TxDone == NULL\r\n");
