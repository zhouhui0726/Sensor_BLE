...
*
* Public function defined in JC_peripheral.h.
*/
...
static void gapRole_processStackMsg(ICall_Hdr *pMsg)
{
...
#ifdef UART_PRINT_ENABLE	
          printS("Connection Parameter Update Response.");
#endif
        }
      }
    }
  ...
  case HCI_GAP_EVENT_EVENT:
    if (pMsg->status == HCI_COMMAND_COMPLETE_EVENT_CODE)
    {
      ...
      {
        VOID memcpy(gapRole_bdAddr, &pPkt->pReturnParam[1], B_ADDR_LEN);
      }
    }
    break;
...
static void gapRole_processGAPMsg(gapEventHdr_t *pMsg)
{
...
#ifdef UART_PRINT_ENABLE			
        printS("Connection establishment!");
        printS("Store connection information!");
#endif
...
#ifdef UART_PRINT_ENABLE			
      System_printf("Connection Terminate reason: %d\r\n", pPkt->reason);
#endif
      memset(gapRole_ConnectedDevAddr, 0, B_ADDR_LEN);
#ifdef UART_PRINT_ENABLE			
      // Erase connection information
      printS("Erase connection information!");
#endif
  ...
  {
    // Notify the application with the new state change
    if (pGapRoles_AppCGs && pGapRoles_AppCGs->pfnEventCB)
    {
      pGapRoles_AppCGs->pfnEventCB((gapPeripheralObserverRoleEvent_t *)pMsg);
    }
  }
#endif
...
static void gapRole_SetupGAP(void)
{
  VOID GAP_DeviceInit(selfEntity, gapRole_profileRole, gapRoleMaxScanRes, gapRole_IRK,
...
*/
bStatus_t GAPObserverRole_StartDiscovery(uint8_t mode, uint8_t activeScan, uint8_t whiteList) //ZH
{
