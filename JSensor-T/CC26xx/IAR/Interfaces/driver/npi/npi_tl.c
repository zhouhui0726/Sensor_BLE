...
#include "util.h"
extern Clock_Struct NPI_Timeout_Clock;
// ****************************************************************************
// defines
// ****************************************************************************
...
void NPITL_handleMrdyEvent(void)
{
    ...
    // transmission
    if ( PIN_getInputValue(MRDY_PIN) == 0 || npiTxActive )
    {
        transportMrdyEvent();
        SRDY_ENABLE();
        Util_startClock(&NPI_Timeout_Clock);
    }
...
static void NPITL_transmissionCallBack(uint16 Rxlen, uint16 Txlen)
{
    ...
    if(Rxlen < NPI_TL_BUF_SIZE && Rxlen > 0)
    {
        if ( taskRxCB )
        {
            taskRxCB(Rxlen);
    ...
    if(Txlen < NPI_TL_BUF_SIZE && Txlen > 0)
    {
        npiTxActive = FALSE;
...
//#ifdef POWER_SAVING
    if (!PTM_Uart_enable) {
      ...
      NPITL_relPM();
      if (Util_isActive(&NPI_Timeout_Clock)) {
        Util_stopClock(&NPI_Timeout_Clock);
      }
      SRDY_DISABLE();
    }
//#endif
